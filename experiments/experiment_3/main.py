...
def evaluate_model(model, test_loader, criterion, device, scaler):
    ...
    with torch.no_grad():
        for batch_X, batch_y in test_loader:
            ...
            pred_features = np.zeros((scaled_pred.shape[0], 5))
            pred_features[:, 3] = scaled_pred.flatten()  # Changed to index 3 for Close price

            target_features = np.zeros((scaled_target.shape[0], 5))
            target_features[:, 3] = scaled_target  # Changed to index 3 for Close price

            price_pred = scaler.inverse_transform(pred_features)[:, 3]
            price_target = scaler.inverse_transform(target_features)[:, 3]
...
def plot_predictions(model, val_loader, test_loader, scaler, device, results_dir, original_data, sequence_length):
    model.eval()
    val_predictions = []
    val_actuals = []
    test_predictions = []
    ...
    with torch.no_grad():
        for batch_X, batch_y in val_loader:
            ...
            val_predictions.extend(price_pred)
            val_actuals.extend(price_target)
    ...
    with torch.no_grad():
        for batch_X, batch_y in test_loader:
            ...
            test_predictions.extend(price_pred)
            test_actuals.extend(price_target)
    # Get dates
    train_size = int(len(original_data) * 0.7)
    val_size = int(len(original_data) * 0.15)

    val_dates = original_data.index[train_size:train_size + val_size][sequence_length:]
    test_dates = original_data.index[train_size + val_size:][sequence_length:]
    ...
    # Validation plot
    plt.subplot(2, 1, 1)
    plt.plot(val_dates, val_actuals, label='Actual Price', color='blue')
    plt.plot(val_dates, val_predictions, label='Predicted Price', color='red')
    plt.title('Validation Set: Bitcoin Price Actual vs Predicted')
    plt.xlabel('Date')
    plt.ylabel('Price ($)')
    plt.legend()
    plt.grid(True)
    ...
    # Test plot
    plt.subplot(2, 1, 2)
    plt.plot(test_dates, test_actuals, label='Actual Price', color='blue')
    plt.plot(test_dates, test_predictions, label='Predicted Price', color='red')
    plt.title('Test Set: Bitcoin Price Actual vs Predicted')
    plt.xlabel('Date')
    plt.ylabel('Price ($)')
    plt.legend()
    plt.grid(True)
