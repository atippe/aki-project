    ...
    def forward(self, x):
        ...
        for t in range(seq_sz):
            ...
            for layer in range(self.num_layers):
                ...
                # Apply GLU with residual connection and scaling
                glu_out = self.glu(h_t[layer])
                h_t[layer] = h_t[layer] + 0.1 * glu_out

                # Scale skip connection
                skip = 0.1 * self.skip_connections[layer](layer_input)
        ...
        # Scale additional processing components
        hidden_seq = hidden_seq + 0.1 * self.self_attention(hidden_seq)
        hidden_seq = hidden_seq + 0.1 * self.temporal_conv(hidden_seq)
...
class MultiStepPricePredictionModel(nn.Module):
    def __init__(self, input_size, hidden_size, output_steps, num_layers=2, dropout=0.2):
        ...
        # Enhanced prediction head for multiple steps
