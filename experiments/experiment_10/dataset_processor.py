...
class BasicDataProcessor:
    ...
    def create_sequences(self, data, seq_length=60, target_feature='Close', prediction_steps=5):
        """
        ...
        self.logger.info(f"Prediction steps: {prediction_steps}")

        # Adjust range to account for both sequence length and prediction steps
        for i in range(len(data_array) - seq_length - prediction_steps + 1):
            sequence = data_array[i:(i + seq_length)]
            # Get multiple future targets
            target = data_array[(i + seq_length):(i + seq_length + prediction_steps), target_idx]
    ...
    def process(self, start_date='2022-01-01', resample_rule='1h', agg_dict=None, seq_length=60, features=None, target_feature=None, split_ratios=(0.7, 0.15, 0.15), prediction_steps=5):
        """
        ...
        train_log = np.log1p(train_data[features])
        val_log = np.log1p(val_data[features])
        test_log = np.log1p(test_data[features])

        self.scaler.fit(train_log)

        train_scaled = pd.DataFrame(
            self.scaler.transform(train_log),
            columns=features,
            index=train_data.index
        )
        val_scaled = pd.DataFrame(
            self.scaler.transform(val_log),
            columns=features,
            index=val_data.index
        )
        test_scaled = pd.DataFrame(
            self.scaler.transform(test_log),
            columns=features,
            index=test_data.index
        )

        self.logger.info(f"\nCreating sequences (length={seq_length}, prediction_steps={prediction_steps})...")

        self.logger.info("\nCreating training sequences...")
        X_train, y_train = self.create_sequences(train_scaled, seq_length, prediction_steps=prediction_steps)

        self.logger.info("\nCreating validation sequences...")
        X_val, y_val = self.create_sequences(val_scaled, seq_length, prediction_steps=prediction_steps)

        self.logger.info("\nCreating testing sequences...")
        X_test, y_test = self.create_sequences(test_scaled, seq_length, prediction_steps=prediction_steps)
    ...
    def save_processed_data(self, X_train, y_train, X_val, y_val, X_test, y_test, original_data, seq_length, features, prediction_steps):
